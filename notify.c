...
//		strcat(buf, buf_temp);
	...
	for (ct_s = 0; ct_s < in_port_n - 1; ct_s++) {
		...
		sprintf(buf_temp, "{value:\"%d\",text:\"Вход%d\"},", ct_s, ct_s + 1);
		strcat(buf, buf_temp);
	...
	for (ct_s = 0; ct_s < sensor_cont - 1; ct_s++) {
		...
		sprintf(buf_temp, "{value:\"%d\",text:\"Термодатчик%d\"},",
				ct_s + start_ct, ct_s + 1);
		strcat(buf, buf_temp);
	}

	sprintf(buf_temp, "{value:\"%d\",text:\"Термодатчик%d\"}",
			sensor_cont + start_ct - 1, sensor_cont);
	strcat(buf, buf_temp);
#endif
...
void load_def_notify(void) {
	...
	sprintf((char*) NOTF[0].name, "Контроль входной двери");
...
//	sprintf((char*) NOTF[0].signal, "input0");
	sprintf((char*) NOTF[0].text_mess,
			"Концевой выключатель входной двери замкнут");
...
void log_swich_notf(char *out, log_reple_t *input_reply) {
	...
	case NOTF_SETT:
		...
		break;
	case NOTF_ERR:
		sprintf(out_small, "%s %s", "Ошибка модуля: \n\r",
				NOTF[input_reply->line].name);
		break;
	default:
...
//							IN_PORT[NOTF[nf_ct].signal_n].sost_filtr);
					...
					event_nf[nf_ct] = notife_run(ex_data,
							IN_PORT[NOTF[nf_ct].signal_n].sost_filtr);

				//	printf("event_nf=%d\n\r", event_nf[nf_ct]);
					if ((event_nf[nf_ct] != 0) && (event_nf_old[nf_ct] == 0)) {
						event_nf[nf_ct] = 1;
						event_nf_old[nf_ct] = 1;
						log_notf_save_mess(NOTF_ACTIV, nf_ct);
					} else {
						if (event_nf[nf_ct] == 0) {
							event_nf_old[nf_ct] = 0;
						} else {
							event_nf[nf_ct] = 0;
						}
				...
				if (NOTF[nf_ct].signal_n < (in_port_n + out_port_n)) {
					event_nf[nf_ct] =
							notife_run(ex_data,
			...
			//	printf("NOTF[%d].method_n=%d\n\r", nf_ct, NOTF[nf_ct].method_n);
				...
				if ((event_nf[nf_ct] == 1) && (NOTF[nf_ct].method_n == 0)) {
...
//							FW_data.smtp.V_EMAIL_CC1, FW_data.smtp.V_EMAIL_CC2);
			...
			//		if (NOTF[nf_ct].name[0] == 0) {
				//		printf("NOTF[nf_ct].name[0] == 0\n\r");
				    ...
				    long int mac;
				    struct netif *_netif;
				    esp_err_t ret = tcpip_adapter_get_netif(TCPIP_ADAPTER_IF_ETH,(void**) &_netif);
				    mac=_netif->hwaddr[0]|_netif->hwaddr[1]<<8|_netif->hwaddr[2]<<16|_netif->hwaddr[3]<<24;

						if (NOTF[nf_ct].signal_n < in_port_n) {
							sprintf(subj, "NetPing %d Вход%d : %s #%08lx",	serial_id, NOTF[nf_ct].signal_n+1,	NOTF[nf_ct].name,(timeup+mac));
						} else if (NOTF[nf_ct].signal_n
								< (in_port_n + out_port_n)) {
							sprintf(subj, "NetPing %d Выход%d : %s #%08lx",	serial_id, NOTF[nf_ct].signal_n+1 - in_port_n,  NOTF[nf_ct].name,(timeup+mac));
						}else if (NOTF[nf_ct].signal_n	< (in_port_n + out_port_n+max_sensor))
						{
							sprintf(subj, "NetPing %d Термодатчик%d : %s #%08lx",	serial_id, NOTF[nf_ct].signal_n - in_port_n-out_port_n+1,  NOTF[nf_ct].name,(timeup+mac));
//							send_mess_trap_termo(&(ip_trap.addr),
//								(NOTF[nf_ct].signal_n - in_port_n-out_port_n));
						}
...
//					}
				...
				if ((event_nf[nf_ct] == 1) && (NOTF[nf_ct].method_n == 1)) {
...
//						ipt[3]=(NOTF[nf_ct].send_to[11]-0x30)*100+(NOTF[nf_ct].send_to[12]-0x30)*10+(NOTF[nf_ct].send_to[13]-0x30);
					...
					if (err != ERR_OK) {
						...
						IP4_ADDR(&ip_trap, ipt[0], ipt[1], ipt[2], ipt[3]);
//						ip_syslog1.type = 4;
